...
> **Note**: `person.create.proto` and `person.update.proto` are generated by the `gen-proto` command in Step 2. Do not write them manually.

---

### Step 2: Generate derived proto files

`gen-proto` reads `gcode.update_message` / `gcode.create_message` annotations and generates derived proto files:
...
```
proto/
  person.proto              ← original (unchanged)
  person_service.proto      ← original (unchanged)
  person.update.proto       ← generated: PersonUpdateByName message
  person.create.proto       ← generated: PersonCreate message
```

> **Note**: `gcode gen-proto` overwrites existing `*.update.proto` / `*.create.proto` files on every run. Do not manually edit these generated files — changes will be lost on the next run.
...
```
dao/
  person.pb.dao.go              ← Person struct + serialization methods
  person.pb.dao.validate.go     ← Person.Validate() method
...
>
> **Validate inheritance**: Derived message `Validate()` automatically inherits rules from the source message. Optional fields (pointer types) skip validation when nil; condition_fields are validated without a zero-value guard. See [Annotations Reference — Validate Inheritance](annotations.md#validate-inheritance).

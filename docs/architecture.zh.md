...
```
.meta.proto 文件（schema 源——定义 validate 规则、update_message / create_message 注解）
    │
    ▼
[gen-proto]     前置子命令：读取 .meta.proto schema 文件，
                为每个 schema 生成三类 proto 文件：
                  *.entity.proto  — struct 定义（无 validate，有 gorm）
                  *.create.proto  — create message（validate 注解从 schema 拷贝）
                  *.update.proto  — update message（validate 注解从 schema 拷贝）
                所有生成文件写入同一目录
    │
    ▼
.proto 文件（entity + create + update + service）
    │
    ▼
[source]        扫描目录，发现所有 .proto 文件，稳定排序
                （跳过 .meta.proto 文件——仅由 gen-proto 消费）
    │
    ▼
...
[render]        Go 中间表示 → Go 源码字符串，go/format 格式化，输出 []byte
    ...
    ▼
生成文件
  *.entity.pb.dao.go              struct 定义 + MarshalBinary/UnmarshalBinary
  *.entity.pb.dao.validate.go     Validate()（返回 nil，entity 无 validate 注解）
  *.create.pb.dao.go              struct + ToEntity()
  *.create.pb.dao.validate.go     Validate()（直接从 create proto 字段读取规则）
  *.update.pb.dao.go              struct + ToMap() + ApplyTo()
  *.update.pb.dao.validate.go     Validate()（直接从 update proto 字段读取规则）
  *.pb.rpc.go                     service interface 定义
  *.pb.http.go                    gin HTTP handler 工厂函数
```

TypeScript 生成使用并行流水线，共用相同的 parser 和 transform 阶段。
`.meta.proto` 文件被跳过，仅处理 `*.entity.proto`、`*.create.proto`、`*.update.proto` 和普通 proto 文件：
...
[tsrender]       Go 中间表示 → TypeScript 源码字符串，
    ...
    ▼
生成文件
  *.entity.pb.ts           interface、enum、enum 名称映射（无验证元数据）
  *.create.pb.ts           interface + XxxRules 验证元数据
  *.update.pb.ts           interface + XxxRules 验证元数据
...
Go 生成会把所有 Go 文件写入 `-out` 指定的单一目录。这与 render 层的同 package 类型模型一致：同一个 `go_package` 内的跨文件类型会生成为未限定 Go 类型名。一次生成中，同 basename 的 proto 文件会被拒绝，因为它们会产生相同的平铺输出文件名。
...
| 文件                          | 触发条件                    | 内容                                                                                                                                                  |
| ----------------------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `*.entity.pb.dao.go`          | `*.entity.proto` 文件       | struct 定义、json/gorm tag、MarshalBinary、UnmarshalBinary、UnmarshalBinaryLenient、TableName()（有 gorm.table 注解时） |
| `*.entity.pb.dao.validate.go` | `*.entity.proto` 文件       | `Validate() error`——返回 nil（entity proto 无 validate 注解） |
| `*.create.pb.dao.go`          | `*.create.proto` 文件       | struct 定义、ToEntity() |
| `*.create.pb.dao.validate.go` | `*.create.proto` 文件       | `Validate() error`——直接从 create proto 字段读取规则 |
| `*.update.pb.dao.go`          | `*.update.proto` 文件       | struct 定义、ToMap()、ApplyTo() |
| `*.update.pb.dao.validate.go` | `*.update.proto` 文件       | `Validate() error`——直接从 update proto 字段读取规则 |
| `*.pb.dao.go`                 | 普通 proto 文件             | struct 定义、json/gorm tag、MarshalBinary、UnmarshalBinary、UnmarshalBinaryLenient、ToMap（update 派生 message）、TableName()（有 gorm.table 注解时） |
| `*.pb.dao.validate.go`        | 普通 proto 文件             | `Validate() error` 方法，覆盖全部 buf/validate 约束类型                                                                                               |
...
| `*.pb.ts`                     | `gcode gen-ts` 子命令       | TypeScript interface、enum、enum 名称映射、验证元数据（entity 文件无验证元数据）、跨文件 ES module import                                             |
...
6. **validate 规则显式写入 proto** — validate 注解在 `.meta.proto` 中定义，由 `gen-proto` 拷贝到 `*.create.proto` / `*.update.proto`。render 层直接从这些 proto 字段读取规则，无需跨文件反查
